...
public class NormalizeData {
	...
	public static void main(String[] args) {
		...
		for(int k=2; k<=1;k++){
			...
			try{//get image
				File imgf = new File("faces/face" + k + ".jpg");
			...
			try {
			    // retrieve image
			    BufferedImage bi = sImg;
			    File outputfile = new File("normalizedFaces/face"+k+".jpg");
		...
		for(int k=0; k<=10019;k++){
			System.out.println(k);
			BufferedImage img = null;
			
			try{//get image
				File imgf = new File("background/" + k + ".jpg");
				img = ImageIO.read(imgf);
			}
			catch (IOException e) {
			  }
			...
			try {
			    // retrieve image
			    BufferedImage bi = sImg;
			    File outputfile = new File("normalizedBackground/"+k+".jpg");
			    ImageIO.write(bi, "jpeg", outputfile);
			} catch (IOException e) {
